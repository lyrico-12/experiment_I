...
void cut_wave(complex double * X1, complex double * X2, long n) {
    long sample_rate = 5000;
    for (long i = 0; i < n; i++) {
        if (i % (sample_rate * 2) < sample_rate) {
            X2[i] = X1[i];
            X1[i] = 0;
        } else {
            X2[i] = 0;
...
void union_wave(complex double * X1, complex double * X2, long n) {
    for (long i = 0; i < n; i++) {
        if (X1[i] == 0) {
            X1[i] = X2[i];
        }
...
void vibrato(complex double * Y, long n, double vibrato_rate) {
    complex double *Z = calloc(sizeof(complex double), n);
    for (long i = 0; i < n; i++) {
        long id = (long)(vibrato_rate * i);
        Z[i] = Y[id % n];
    }
    for (long i = 0; i < n; i++) {
        Y[i] = Z[i];
...
int main(int argc, char ** argv) {
  ...
  long n = atol(argv[1]);
  double vibrato_rate = atol(argv[2]);
  ...
  complex double * X1 = calloc(sizeof(complex double), n);
  complex double * X2 = calloc(sizeof(complex double), n);
